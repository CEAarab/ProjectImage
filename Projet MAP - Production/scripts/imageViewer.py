...
class ImageViewer(tk.Canvas):
	...
	'''
	# Associée à la fonction convertImage8bits, permet de passer de l'image float64 en uint8
	def relu_255(self, x) :
		return np.maximum(0, np.minimum(x, 255)) # Resserre toutes les valeurs qui sortaient de la plage 0-100
	'''

	# Converti l'image en 8 bits si besoin (uint16 est mauvais pour l'affichage)
	def convertImage8bits(self, img):
		'''
		if img.dtype == 'float64': # chronophage et inutile pour l'affichage
			return (self.relu_255(img)).astype(np.uint8)
		'''
		if img.dtype == 'uint16' :
